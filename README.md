...
A 2D platformer motor that handles mechanics such as double jump, wall jumps, and corner grabs. Includes a player controlled prefab that can be dropped into any scene for immediate support.
...
For immediate player support, drop the PlatformerPlayer2D prefab into the scene. To leverage PlatformerMotor2D add the component to an existing GameObject. Interact with the Motor using methods and members.
